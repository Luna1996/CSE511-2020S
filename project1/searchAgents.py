...
def cornersHeuristic(state, problem):
  ...
  while corners != []:
    distances = []
    for c in corners:
      distances.append(util.manhattanDistance(c, reference))
    h += min(distances)
...
def foodHeuristic(state, problem):
  ...
  if n == 0:
    return 0
  wc = problem.walls.count()
  if wc != 80:
    return 1

  H = 0
  for i in range(n):
    dis = [D(pos, food) for food in foods]
    mind = min(dis)
    mini = dis.index(mind)
    H += mind
    pos = foods[mini]
...
class ContestProblem(FoodSearchProblem):
  def isGoalState(self, state):
    pos, _ = state
    x, y = pos
    return self.food[x][y]
...
def contestHeuristic(state, problem):
  ...
  w = grid.width
  h = grid.height
  label = [[0 for j in range(h)] for i in range(w)]
  ...
  for x in range(1, w):
    for y in range(1, h):
      if grid[x][y]:
        if grid[x-1][y] or grid[x][y-1]:
          if grid[x-1][y] and grid[x][y-1]:
            if label[x-1][y] != label[x][y-1]:
              equal.append((label[x-1][y], label[x][y-1]))
  ...
  # second pass
  center = [[0, 0] for i in range(n + 1)]
  count = [0 for i in range(n + 1)]
  for x in range(w):
    for y in range(h):
      if label[x][y] != 0:
        label[x][y] = trans[label[x][y]]
        center[label[x][y]][0] += x
        center[label[x][y]][1] += y
        count[label[x][y]] += 1
  N = 0
  G = []
  for i in range(1, n+1):
    if count[i] > 0:
      N += 1
      G.append(((center[i][0]/count[i], center[i][1]/count[i]), count[i]))

  H = 0
  for i in range(N):
    dis = [D(pos, c[0]) for c in G]
    mind = min(dis)
    mini = dis.index(mind)
    H += mind * G[mini][1]
    pos = G[mini][0]
...
def minComponent(grid, pos):
  ...
  for x in range(w):
    for y in range(h):
      if label[x][y] != 0:
        label[x][y] = trans[label[x][y]]
        d = util.manhattanDistance(pos, (x, y))
        dis[label[x][y]] += d
        count[label[x][y]] += 1
  for i in range(1, n+1):
    dis[i] /= count[i]

  # minimal component retrieve
  m = (float('inf'), 0)
  for i in range(1, n+1):
    if count[i] != 0 and dis[i] < m[0]:
      m = (dis[i], i)
  G = Grid(w, h)
  for x in range(w):
    for y in range(h):
